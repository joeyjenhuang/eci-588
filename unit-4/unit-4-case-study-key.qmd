...
accum.ena <- ena.accumulate.data(
  text_data    = RS.data %>% pull(text),
  units        = data %>% select(all_of(unitCols)),
  conversation = data %>% select(all_of(conversationCols)),
  metadata     = data %>% select(all_of(metaCols)),
  codes        = data %>% select(all_of(codeCols)),
  window.size.back = 7
)
...
set.ena <- ena.make.set(
  ...
  rotation.params = data %>%
    pull(Condition) %>%
    { list(. == "FirstGame", . == "SecondGame") }
)
```

## 3. EXPLORE

Now that we have constructed an ENA Model object, we can begin explore what is stored in the `set.ena` object by typing `set$` and selecting items from the drop down list.

First, let's use the `names()` function to first determine what items and data summaries are stored in the `set.ena` object:
...
Now let's take a look at some of these items that are often of interest.
...
We can also explore descriptive summaries stored in the `set.ena` object and used to constructiont the visualizations above by simply typing `set$` and selecting items from the drop down list.

#### Connection counts

For each unit, ENA creates a cumulative adjacency vector that contains the sums of all unique code co-occurrences for that unit across all stanza windows. **Connection counts** are the frequencies of unique connections a unit made among all possible code pairs.
...
Use the code chunk below to load the {skimr} library and `skim()` the `set.ena$connection.counts` to get a descriptive summary:
...
| \<chr\>               | \<chr\>   | \<chr\>     | \<chr\>           |   |
+:======================+:==========+:============+:==================+===+
| FirstGame.steven z    | FirstGame | steven z    | 1                 |   |
+-----------------------+-----------+-------------+-------------------+---+
| FirstGame.akash v     | FirstGame | akash v     | 2                 |   |
+-----------------------+-----------+-------------+-------------------+---+
| FirstGame.alexander b | FirstGame | alexander b | 1                 |   |
+-----------------------+-----------+-------------+-------------------+---+
| FirstGame.brandon l   | FirstGame | brandon l   | 1                 |   |
+-----------------------+-----------+-------------+-------------------+---+
| FirstGame.christian x | FirstGame | christian x | 0                 |   |
+-----------------------+-----------+-------------+-------------------+---+
| FirstGame.jordan l    | FirstGame | jordan l    | 2                 |   |
+-----------------------+-----------+-------------+-------------------+---+
| FirstGame.arden f     | FirstGame | arden f     | 1                 |   |
+-----------------------+-----------+-------------+-------------------+---+
| FirstGame.margaret n  | FirstGame | margaret n  | 3                 |   |
+-----------------------+-----------+-------------+-------------------+---+
| FirstGame.connor f    | FirstGame | connor f    | 0                 |   |
+-----------------------+-----------+-------------+-------------------+---+
| FirstGame.jimmy i     | FirstGame | jimmy i     | 2                 |   |
+-----------------------+-----------+-------------+-------------------+---+

1-10 of 48 rows \| 1-4 of 22 columns
...
| \<chr\>                                                 | \<dbl\>       |   |
+:========================================================+==============:+===+
| Data & Technical.Constraints                            | -0.2971401126 |   |
+---------------------------------------------------------+---------------+---+
| Data & Performance.Parameters                           | 0.1467451479  |   |
+---------------------------------------------------------+---------------+---+
| Technical.Constraints & Performance.Parameters          | 0.0062512946  |   |
+---------------------------------------------------------+---------------+---+
| Data & Client.and.Consultant.Requests                   | -0.0066785510 |   |
+---------------------------------------------------------+---------------+---+
| Technical.Constraints & Client.and.Consultant.Requests  | -0.2274653715 |   |
+---------------------------------------------------------+---------------+---+
| Performance.Parameters & Client.and.Consultant.Requests | 0.0395312676  |   |
+---------------------------------------------------------+---------------+---+
| Data & Design.Reasoning                                 | 0.1692691629  |   |
+---------------------------------------------------------+---------------+---+
| Technical.Constraints & Design.Reasoning                | 0.0314608661  |   |
+---------------------------------------------------------+---------------+---+
| Performance.Parameters & Design.Reasoning               | 0.4197206428  |   |
+---------------------------------------------------------+---------------+---+
| Client.and.Consultant.Requests & Design.Reasoning       | 0.0003563438  |   |
+---------------------------------------------------------+---------------+---+

1-10 of 15 rows \| 1-2 of 16 columns
...
1-10 of 48 rows \| 1-4 of 7 columns
